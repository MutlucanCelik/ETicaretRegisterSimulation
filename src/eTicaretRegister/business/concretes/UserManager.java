package eTicaretRegister.business.concretes;



import java.util.Scanner;

import eTicaretRegister.business.abstracts.UserService;
import eTicaretRegister.core.abstracts.GoogleUserInformationService;
import eTicaretRegister.core.concretes.Validators;
...
public class UserManager implements UserService{
	...
	public void add(User user) {
		...
		if(!Validators.isUsedMail(userDao.getAll(),user)) {
			...
			if(Validators.isMailConfirmation(true)) {
				System.out.println(user.getName() + " tebrikler başarıyla kayıt oluşturuldu.");
				userDao.add(user);
	...
	public void registerWithGoogle(GoogleUserInformationService info) {
		...
		user.setName(info.getUserInfo().getName());
		...
		if(!Validators.isUsedMail(userDao.getAll(),user)) {
			...
			if(Validators.isMailConfirmation(true)) {
				System.out.println(user.getName() + " tebrikler google hesabımız ile başarıyla kayıt oluşturuldu.");
				userDao.add(user);
